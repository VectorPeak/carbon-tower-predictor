# -*- coding: utf-8 -*-
"""
步骤 5 V3 终极版：适配差分预测的大容量网络训练
核心升级：
1. 读取差分数据集 (Y_diff)，模型直接学习温度的变化趋势。
...
def train_model():
    ...
    Y_train_diff = torch.FloatTensor(train_data['Y_diff']).to(device)  # 🚨 核心改变：读取差分标签
